...
export default function Watch({
  ...
  const onPlay = () => {
    const row = {
      video_id: id,
      channel_id: data ? channelIdFromUrl(data.uploaderUrl) : undefined,
    ...
    <div className="max-w-[1720px] mx-auto px-3 sm:px-6 pt-4 lg:pt-6 flex flex-col lg:flex-row gap-6">
      <div className="flex-1 min-w-0">
        ...
        <div className="flex flex-wrap items-center gap-3 mt-3">
          <div className="flex items-center gap-3 me-auto">
            ...
            <button onClick={() => onChannel(channelId, data.uploader)} className="leading-tight text-start">
              <div className="flex items-center gap-1 font-bold text-[15px]">
                {data.uploader}
                {data.uploaderVerified && <BadgeCheck className="w-4 h-4 text-yt-sub" />}
              </div>
              <div className="text-xs text-yt-sub">{fmtViews(data.uploaderSubscriberCount)} مشترك</div>
            </button>
            <button
              onClick={() => onToggleSub({ channelId, name: data.uploader, avatar: data.uploaderAvatar })}
              className={`ms-2 h-9 px-4 rounded-full text-sm font-bold transition-all active:scale-95 ${
                isSubscribed(channelId) ? "bg-yt-surface text-yt-text" : "bg-yt-text text-yt-bg hover:bg-white/80"
              }`}
            >
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <div className="flex items-center bg-yt-surface rounded-full overflow-hidden">
              <button
                onClick={() => { onToggleLike(); setDisliked(false); }}
              ...
              <button
                onClick={() => { setDisliked((d) => !d); if (liked) onToggleLike(); }}
            ...
            <button onClick={() => notify("تم نسخ الرابط 🔗")} className="flex items-center gap-2 h-9 px-3.5 rounded-full bg-yt-surface hover:bg-yt-hover text-sm font-medium">
              <Share2 className="w-5 h-5" /> مشاركة
            </button>
            <button onClick={() => notify("بدأ التنزيل في الخلفية")} className="flex items-center gap-2 h-9 px-3.5 rounded-full bg-yt-surface hover:bg-yt-hover text-sm font-medium">
            ...
            >
              <ListPlus className="w-5 h-5" /> <span className="hidden xl:inline">{saved ? "محفوظ" : "حفظ"}</span>
            </button>
            <button className="w-9 h-9 rounded-full bg-yt-surface hover:bg-yt-hover grid place-items-center" aria-label="المزيد">
        ...
        <div className="mt-4 bg-yt-surface rounded-xl p-3 text-sm">
          ...
          <p className={`mt-2 leading-relaxed text-yt-text/90 whitespace-pre-line ${expanded ? "" : "line-clamp-2"}`}>
            {data.description || "لا يوجد وصف لهذا الفيديو."}
          </p>
          {data.description && (
            <button onClick={() => setExpanded((e) => !e)} className="font-bold mt-1 text-yt-sub hover:text-yt-text">
        ...
        <section className="mt-6">
          ...
          <div className="flex gap-3 mt-4">
            <span className="w-10 h-10 rounded-full grid place-items-center text-sm font-bold bg-gradient-to-br from-yt-blue to-teal-400 text-black shrink-0">أ</span>
            <input
      ...
      <aside className="lg:w-[400px] xl:w-[420px] shrink-0">
          ...
          {(["الكل", "من القناة", "قصيرة"] as const).map((t) => (
            <button
              ...
              className={`h-8 px-3 rounded-lg text-sm font-medium shrink-0 transition-colors ${
                relFilter === t ? "bg-yt-text text-yt-bg font-bold" : "bg-yt-surface hover:bg-yt-hover"
              }`}
            >
              {t}
            </button>
        ...
        <div className="space-y-3">
          {related.map((r) => (
            <button key={r.url} onClick={() => onOpen(r)} className="w-full flex gap-2.5 group text-start">
              <div className="relative w-[168px] aspect-video rounded-lg overflow-hidden bg-yt-raised shrink-0">
                <img src={r.thumbnail} alt={r.title} loading="lazy" referrerPolicy="no-referrer" className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" />
                <span className="absolute bottom-1 end-1 bg-black/80 text-white text-[11px] font-medium px-1 py-0.5 rounded">{fmtDuration(r.duration)}</span>
              </div>
              <div className="flex-1 min-w-0">
...
function CommentRow({ c }: { c: { author: string; thumbnail: string; commentText: string; commentedTime: string; likeCount: number; replyCount?: number } }) {
    ...
    <div className="flex gap-3">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-4 mt-2 text-yt-sub">
          <button onClick={() => setLiked((l) => !l)} className="flex items-center gap-1.5 text-xs hover:text-yt-text">
            <ThumbsUp className={`w-4 h-4 ${liked ? "fill-current text-yt-blue pop" : ""}`} />
            {c.likeCount + (liked ? 1 : 0)}
          </button>
          <button className="hover:text-yt-text" aria-label="لم يعجبني"><ThumbsDown className="w-4 h-4" /></button>
